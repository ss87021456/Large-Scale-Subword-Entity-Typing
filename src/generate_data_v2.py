...
# python ./src/generate_data_v2.py --input=./data/smaller_preprocessed_sentence_keywords_labeled.tsv
...
def run(model_dir, input, testing):
    ...
    pkl.dump(X_t, open(model_dir + "training_data_v2.pkl", 'wb'))
    pkl.dump(X_te, open(model_dir + "testing_data_v2.pkl", 'wb'))
    del X_t, X_te
    ...
    pkl.dump(X_m_t, open(model_dir + "training_mention_v2.pkl", 'wb'))
    pkl.dump(X_m_te, open(model_dir + "testing_mention_v2.pkl", 'wb'))
    del X_m_t, X_m_te
    ...
    pkl.dump(y_train, open(model_dir + "training_label_v2.pkl", 'wb'))
    pkl.dump(y_test, open(model_dir + "testing_label_v2.pkl", 'wb'))
    del y_train, y_test

    print("dumping pickle file of tokenizer/m_tokenizer/mlb...")
    # dumping model
    pkl.dump(tokenizer, open(model_dir + "tokenizer_v2.pkl", 'wb'))
    pkl.dump(m_tokenizer, open(model_dir + "m_tokenizer_v2.pkl", 'wb'))
    pkl.dump(mlb, open(model_dir + "mlb_v2.pkl", 'wb'))
